...
def load_data():
    ...
    # Load employees
    try:
        print(f"Loading employees from: {paths['employees']}")
        ...
        for emp in employees:
            try:
                ...
                # Bypass specific attribute error
                print(f"Warning: Could not register employee {emp.employeeID} due to: {ae}")
                # Store employee directly in lists instead of using register methods
                if isinstance(emp, Teacher):
                    if not hasattr(school, 'teacher_list'):
                        school.teacher_list = []
                    school.teacher_list.append(emp)
                else:
                    if not hasattr(school, 'employee_list'):
                        school.employee_list = []
                    school.employee_list.append(emp)
        
        # Report how many were registered successfully vs bypassed
        print(f"Registered {len(school.employees)} employees ({len(school.teachers)} teachers).\n")
    except FileNotFoundError:
        print(f"Error: Employee CSV file not found.")
        sys.exit(1)
    except Exception as e:
        print(f"Error loading employees: {e}")
        print(f"Exception Details: {e.__class__}, {e}")
        sys.exit(1)
    ...
    try:
        ...
        print("Error: One or more grade CSV files not found.")
        sys.exit(1)
    except Exception as e:
...
def add_student_cli(school: School, student_data: Optional[Dict[str, Any]] = None):
    ...
    except ValueError as ve: #Catch value errors specifically
        print(f"Invalid input: {ve}")
    except Exception as e:
        print(f"Error adding student: {e}")


def remove_student_cli(school: School, student_id: str = None):
...
def view_student_cli(school: School, student_id: str = None):
...
def update_student_cli(school: School, student_id: str = None, updates: Dict[str, Any] = None):
...
def update_student_grades_cli(school: School):
...
def delete_student_cli(school: School, student_id: str = None):
...
def search_students_cli(school: School):
...
def add_employee_cli(school: School):
...
def view_employee_cli(school: School):
...
def update_employee_cli(school: School):
...
#     main()
 ...
 # try:
    ...
    #     for emp in employees:
    #         #  --- DEBUGGING ---
    #         print(f"Registering employee: {emp}, Type: {type(emp)}")
    #         print(f"Employee object: {emp.__dict__}")
    #         #  --- END DEBUGGING ---
